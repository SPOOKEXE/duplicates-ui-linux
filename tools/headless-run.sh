#!/usr/bin/env bash
# Drives duplicates-ui on a private Xvfb display, so a UI change can be checked without
...
APP="${APP:-$HERE/../build/duplicates-ui}"
DISP="${DISPLAY_NUM:-:99}"
...
mkdir -p "$OUT"
Xvfb "$DISP" -screen 0 1300x900x24 > /dev/null 2>&1 &
XVFB_PID=$!
sleep 2
...
export DISPLAY="$DISP"
WID=$(xdotool search --name "^duplicates-ui$" | head -1)
if [ -z "$WID" ]; then
